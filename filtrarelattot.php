<?php
	...
	$querycart = "select TOTAL, TIDECOD, TIDEDESC FROM (select format(sum(carflu.caflval),2,'pt_BR') as TOTAL, tidecod as TIDECOD,tidedesc as TIDEDESC from carflu,tipdes where carflu.cafltipo = tipdes.tidecod and carflu.caflfecha = '$ano-$mes' GROUP BY carflu.cafltipo) as tabela1 order by CAST(TOTAL AS unsigned) DESC";
	$querydin = "select TOTAL, TIDECOD, TIDEDESC FROM (select format (sum(fluxo.valor),2,'pt_BR') as TOTAL, tidecod, tidedesc from fluxo,tipdes where fluxo.tipocod = tipdes.tidecod AND fluxo.DATAPAG like '%$ano-$mes%' AND fluxo.CONTA = 'D' GROUP BY fluxo.tipocod ORDER BY tipdes.tidedesc) as tabela1 order by CAST(TOTAL AS unsigned) DESC";	
		...
		<table id=tabela border="1" style="width:80%; text-align:center;">	
			...
			<tbody> ';			
					...
					while($row = mysqli_fetch_array($result)) {
							...
							echo
							"							
							<tr onclick=\"window.open('graflincart.php?mes=$mes&ano=$ano&tipo=".$row['TIDECOD']."', '_blank', 'width=900,height=500') ; mstTabela(".$row['TIDECOD'].")\";>";
							echo
							"
							<td>
							".$row['TIDEDESC']."
