...
<?php
	...
	$querydin = "select format (sum(fluxo.valor),2,'pt_BR') as TOTAL, tidedesc from fluxo,tipdes where fluxo.tipocod = tipdes.tidecod AND fluxo.DATAPAG like '%$ano-$mes%' AND fluxo.CONTA = 'D' GROUP BY fluxo.tipocod ORDER BY tipdes.tidedesc";
	$querycart = "select format(sum(carflu.caflval),2,'pt_BR') as TOTAL, tidedesc from carflu,tipdes where carflu.cafltipo = tipdes.tidecod and carflu.caflfecha = '$mes' GROUP BY carflu.cafltipo";
